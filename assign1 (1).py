...
def func_sort_distance(goal_dictionary): #match a lower token for each upper token with shortest distance
    sorted_goal_dict={}
...
def six_hex_surrond(token): #find the six surronding hex for a given hex
    if len(token)==3:
        token=token[1:3]
    action=[[-1,0],[0,-1],[1,-1],[1,0],[0,1],[-1,1]] #six direction list in clockwise order
    six_hex=[]
    for item in action:
        x=item[0]+token[0]
        y=item[1]+token[1]
    ...
    for i in range(6):
        ...
        if (item in state):
            if (state[item].isupper()): #have upper token --> can swing
                layer2=six_hex_surrond(item) #six hex connected to the upper token for swing
...
def open_close_dict_build(state, upper, sorted_goal_dictionary, close_list): #build initial open_close_dict
    open_close_dict={}
    for u in upper:
        u=tuple(u)
